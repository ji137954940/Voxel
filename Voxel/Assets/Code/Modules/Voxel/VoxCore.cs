...
namespace Color.Number.Voxel
{
    ...
    public class VoxelMesh
    {
        ...
        //            UnityEngine.Color[] _colors = new UnityEngine.Color[width * height];
        ...
        //                            break;
        //                    }
        //                    UnityEngine.Color _c = MainVoxelData.Palatte[colorIndex];
        ...
        //                else
        //                {
        //                    UnityEngine.Color c = MainVoxelData.Palatte[currentIndex];
        //                    _texture = new Texture2D(3, 3, TextureFormat.ARGB32, false, false);
        //                    _texture.SetPixels(new UnityEngine.Color[9] { c, c, c, c, c, c, c, c, c });
    ...
    public class _25DSprite
    {
        ...
        )
        {
            ...
            int[] heights;
            Vector2[] pivots;
            UnityEngine.Color[][] colorss = GetColorss(
        ...
        private static UnityEngine.Color[][] GetColorss(
            Voxel[,,] voxels, UnityEngine.Color[] palette,
        ...
        )
        {
            ...
            UnityEngine.Color[][] colorss = new UnityEngine.Color[spriteNum][];
            ...
            for (int index = 0; index < spriteNum; index++)
            {
                ...
                // Get Colorss
                colorss[index] = new UnityEngine.Color[width * height];
                ...
                // Do some Cheat (｡･∀･)ﾉﾞ
                {
                    ...
                    List<UnityEngine.Color> cheatColors = new List<UnityEngine.Color>();
                    for (int x = 0; x < width; x++)
                    {
                        for (int y = 0; y < height; y++)
                        {
                            UnityEngine.Color c = CheckPixelsAround_Cheat(colorss[index], x, y, width, height);
                ...
                {
                    UnityEngine.Color outlineColor = UnityEngine.Color.black;
            ...
            for (int x = 0; x < voxelSize.x; x++)
            {
                for (int y = 0; y < voxelSize.y; y++)
                {
                    for (int z = 0; z < voxelSize.z; z++)
                    {
                        ...
                        UnityEngine.Color color = palette[vox.ColorIndex <= 0 ? 0 : vox.ColorIndex - 1];
                        for (int i = 0; i < 6; i++)
                        {
                            if (!vox.IsEmpty && vox.Visible[i])
                            {
                                ...
                                face.Color = UnityEngine.Color.Lerp(
                                    color,
                                    (int)face.Type > 3 ? UnityEngine.Color.black : color,
        ...
        private static bool CheckPixelsAround(UnityEngine.Color[] colors, int x, int y, int w, int h)
        {

            // Self
            if (colors[y * w + x] != UnityEngine.Color.clear)
            {
                return false;
            }

            // U
            if (y < h - 1 && colors[(y + 1) * w + x] != UnityEngine.Color.clear)
            {
                return true;
            }

            // D
            if (y > 0 && colors[(y - 1) * w + x] != UnityEngine.Color.clear)
            {
                return true;
            }

            // L
            if (x < w - 1 && colors[y * w + x + 1] != UnityEngine.Color.clear)
            {
                return true;
            }

            // R
            if (x > 0 && colors[y * w + x - 1] != UnityEngine.Color.clear)
            {
                return true;
            }
        ...
        private static UnityEngine.Color CheckPixelsAround_Cheat(UnityEngine.Color[] colors, int x, int y, int w, int h)
        {
            // Self
            if (colors[y * w + x] != UnityEngine.Color.clear)
            {
            ...
            UnityEngine.Color c, color = UnityEngine.Color.clear;
        ...
        private static SpriteResult PackTextures(UnityEngine.Color[][] colorss, int[] widths, int[] heights, Vector2[] pivots)
        {
            ...
            resultInfo.Texture.SetPixels(new UnityEngine.Color[aimWidth * aimHeight]);
    ...
    //        // Save Mat Asset
    //        AssetDatabase.CreateAsset(resultMat, GameUtils.CombinePaths(exportPath, _name + ".mat"));


    //        // Save Texture
    //        string texturePath = GameUtils.CombinePaths(exportPath, _name + ".png");
