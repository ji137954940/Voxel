...
namespace Color.Number.Voxel
{
    ...
    public static class VoxFile
    {
        ...
        private static UnityEngine.Color[] DefaultPallete
        {
            get
            {

                UnityEngine.Color[] c = new UnityEngine.Color[256];
                int index = 0;
                for (int r = 10; r >= 0; r -= 2)
                {
                    for (int g = 10; g >= 0; g -= 2)
                    {
                        for (int b = 10; b >= 0; b -= 2)
                        {
                            if (r + g + b != 0)
                            {
                                c[index] = new UnityEngine.Color((float)r / 10f, (float)g / 10f, (float)b / 10f);
                                index++;
                            }
                        }
                    }
                ...
                for (int i = 14; i > 0; i--)
                {
                    ...
                    c[index] = new UnityEngine.Color((float)i / 15f, 0f, 0f);
                    index++;
                }
                ...
                for (int i = 14; i > 0; i--)
                {
                    ...
                    c[index] = new UnityEngine.Color(0f, (float)i / 15f, 0f);
                    index++;
                }
                ...
                for (int i = 14; i > 0; i--)
                {
                    ...
                    c[index] = new UnityEngine.Color(0f, 0f, (float)i / 15f);
                    index++;
                }
                ...
                for (int i = 14; i > 0; i--)
                {
                    ...
                    c[index] = new UnityEngine.Color((float)i / 15f, (float)i / 15f, (float)i / 15f);
                    index++;
                }

                c[255] = new UnityEngine.Color(0f, 0f, 0f, 1f);
        ...
        public static byte[] GetVoxByte(VoxData _data, UnityEngine.Color[] _palatte)
        {
            ...
            for (int i = 0; i < 256; i++)
            {
                UnityEngine.Color _color = i < _palatte.Length ? _palatte[i] : new UnityEngine.Color();
        ...
        {
            return LoadVoxel(GameUtils.FileToByte(path));
        }
                ...
                using (BinaryReader _br = new BinaryReader(_ms))
                {
                    ...
                    while (_readSize < _childrenSize)
                    {
                        ...
                        else if (_chunkId[0] == 'R' && _chunkId[1] == 'G' && _chunkId[2] == 'B' && _chunkId[3] == 'A')
                        {
                            ...
                            _mainData.Palatte = new UnityEngine.Color[256];
        ...
        static int ReadPalattee(BinaryReader _br, UnityEngine.Color[] _colors)
        {
            int chunkSize = _br.ReadInt32();
            int childrenSize = _br.ReadInt32();
            for (int i = 0; i < 256; ++i)
            {
                _colors[i] = new UnityEngine.Color((float)_br.ReadByte() / 255.0f, (float)_br.ReadByte() / 255.0f, (float)_br.ReadByte() / 255.0f, (float)_br.ReadByte() / 255.0f);
            }
            if (childrenSize > 0)
            {
                _br.ReadBytes(childrenSize);
