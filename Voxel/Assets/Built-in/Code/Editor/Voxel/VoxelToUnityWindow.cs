...
namespace Voxel2Unity {
	...
	public class VoxelToUnityWindow : EditorWindow {
		...
		void CreateGUI () {
			LayoutF(() => {
                ...
                if (GUI.Button(
                    ...
                    "Create Voxel Obj" + TheS(SelectingVoxelFileNum)
                ))
                {
                    // Create Prefab
                    TaskForAll(TaskType.CreateVoxelObj);
                }
			    Space(6);
				...
				if (task == TaskType.CreatePrefab ||
					...
					switch (task) {
						...
						case TaskType.CreateSimple:
							...
							break;
                        case TaskType.CreateVoxelObj:
                            error = CreateVoxelObj(pathData, currentIndex);
                            break;
					}
				} else if (
					task == TaskType.Flip ||
        ...
        public ErrorType CreateVoxelObj(PathData pathData, int currentIndex)
        {
            ...
            if (_ext == ".vox" || _ext == ".qb")
            {
                ...
                if (_ext == ".vox")
                {
                    ...
                    Debug.Log(" 开始时间 " + Time.time + " 耗时 " + (Time.time - time));
                }
                else
                {
                    _chunk = QbFile.LoadQb(objPath).GetVoxData();
                ...
                if (_chunk != null)
                {
                    VoxObj obj = new VoxObj();

                    obj.CreateObj(_chunk, 0);
		...
		private void ForEachSelecting (System.Action<PathData> action) {
		    ...
		    var arr = SeletingObjAndPaths.Values.ToArray();
